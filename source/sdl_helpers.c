/*
    ...
    See sdl_helpers.h for license info.
...
Result SDL_HelperInit(void) {
	...
	WINDOW = SDL_CreateWindow("In-Home-Switching", 0, 0, 1280, 720, SDL_WINDOW_FULLSCREEN);
...
void SDL_DrawRectR(SDLRect rect, int extent_x, int extent_y, SDL_Color colour) {
	SDL_DrawRect(rect.x - extent_x, rect.y - extent_y, rect.width + extent_x * 2, rect.height + extent_y * 2, colour);
}
