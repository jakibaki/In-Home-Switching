...
static void OSK_BlinkText(int fade, int x, int y) {
	SDL_Color FADE_WHITE = FC_MakeColor(255, 255, 255, fade);
	SDL_DrawText(x, y, 30, FADE_WHITE, "|");
}
...
void OSK_Display(char *title, char *msg) {
	...
	while(appletMainLoop()) {	
		...
		SDL_DrawRect(0, 60, 1280, 100, TITLE_BAR_COLOR);

		SDL_DrawImage(icon_back, 40, 86);

		SDL_DrawText(128, 62 + ((100 - title_height)/2), 30, WHITE, title);

		SDL_DrawRect(0, (660 - (text_height * 5)) - 30, 1280, 720 - ((660 - (text_height * 5)) - 30), OSK_BG_COLOUR_DARK);
		...
		if (strlen(osk_buffer) != 0) {
			...
			SDL_DrawTextf((1280 - buf_width) / 2, 230, 30, WHITE, "%s", osk_buffer);
		}

		OSK_BlinkText(transp, (((1280 - buf_width) / 2) + (buf_width_curr + cursor_width)) - 11, 230);
		transp -= 7;
			...
			for (int y = 0; y <= MAX_Y; y++) {
				if (osk_text_shift || osk_text_caps) {
                    SDL_DrawText(((1280 - (text_width * 2)) / 2) + (100 * x), (660 - (text_height * 5)) + (60 * y), 50, (x == osk_pos_x && y == osk_pos_y)? 
                        BUTTON_COLOR : WHITE, osk_textdisp_shift[x + y * 10]);
				}
				else {
                    SDL_DrawText(((1280 - (text_width * 2)) / 2) + (100 * x), (660 - (text_height * 5)) + (60 * y), 50, (x == osk_pos_x && y == osk_pos_y)?
                        BUTTON_COLOR : WHITE, osk_textdisp[x + y * 10]);
				}
			}
		}

		SDL_DrawImage(icon_remove, 1190, 480);
		SDL_DrawImage(icon_accept, 1190, 600);
		...
		if (touchInfo.state == TouchEnded && touchInfo.tapType != TapNone) {
			if (tapped_inside(touchInfo, 40, 86, 108, 114)) {
				osk_buffer[0] = '\0';
				break;
			}
